  ...
  class AdditionalAdvancedScanInformation
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CloudmersiveVirusScanApiClient::AdditionalAdvancedScanInformation` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CloudmersiveVirusScanApiClient::AdditionalAdvancedScanInformation`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'contains_json')
        self.contains_json = attributes[:'contains_json']
      end

      if attributes.key?(:'contains_xml')
        self.contains_xml = attributes[:'contains_xml']
      end

      if attributes.key?(:'contains_image')
        self.contains_image = attributes[:'contains_image']
      end

      if attributes.key?(:'relevant_subfile_name')
        self.relevant_subfile_name = attributes[:'relevant_subfile_name']
      end
    end
